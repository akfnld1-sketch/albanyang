...
window.addEventListener('load', function(){
  ...
  try{
    ...
    if(!hasJobs){
      setTimeout(function(){
        if(typeof showJobTypeSelector === 'function'){
          showJobTypeSelector(true);
        }
      }, 300);
    } else {
      // 직업유형 있으면 UI 적용
...
function renderSettingsPage(){
  ...
  // ── 화면 설정 ──
  html += `<div style="background:var(--surface);border:1px solid var(--border);border-radius:12px;padding:16px 18px;margin-bottom:12px;">
    <div style="font-size:17px;font-weight:700;color:var(--text);margin-bottom:14px;">🎨 화면 설정</div>
    <div style="display:flex;align-items:center;justify-content:space-between;padding:6px 0;">
      <div>
        <div style="font-size:17px;color:var(--text);">다크 모드</div>
        <div style="font-size:15px;color:var(--text3);">현재: ${isDark?'다크':'라이트'} 모드</div>
      </div>
  ...
  html += `<div style="background:var(--surface);border:1px solid var(--border);border-radius:12px;padding:16px 18px;margin-bottom:12px;">
    ...
    <div style="font-size:16px;color:var(--text3);margin-bottom:12px;line-height:1.6;">버그 신고, 기능 건의, 사용 문의를 남겨주세요.<br>확인 후 빠르게 답변드릴게요!</div>
    <div style="margin-bottom:8px;">
      <input id="contact-name" type="text" placeholder="이름 (선택)"
        style="width:100%;box-sizing:border-box;padding:10px 12px;border-radius:8px;border:1px solid var(--border);
               background:var(--surface2);color:var(--text);font-size:18px;font-family:'Noto Sans KR';outline:none;margin-bottom:8px;">
      <input id="contact-email" type="email" placeholder="이메일 (답변 받을 주소)"
        style="width:100%;box-sizing:border-box;padding:10px 12px;border-radius:8px;border:1px solid var(--border);
               background:var(--surface2);color:var(--text);font-size:18px;font-family:'Noto Sans KR';outline:none;margin-bottom:8px;">
      <textarea id="contact-msg" placeholder="문의 내용을 입력해주세요" rows="4"
        style="width:100%;box-sizing:border-box;padding:10px 12px;border-radius:8px;border:1px solid var(--border);
               background:var(--surface2);color:var(--text);font-size:18px;font-family:'Noto Sans KR';
      ...
      <button onclick="submitContact()"
        style="width:100%;padding:11px;border-radius:8px;border:none;background:var(--accent);
               color:#fff;font-size:17px;font-weight:700;cursor:pointer;font-family:'Noto Sans KR';">
        📨 문의 보내기</button>
      <div id="contact-result" style="margin-top:8px;font-size:16px;text-align:center;min-height:16px;"></div>
    </div>
  </div>`;
