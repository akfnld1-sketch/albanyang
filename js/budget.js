...
function renderCalendar(){
  ...
  // ── 근태 v3 위임 (atm2_attV3='off'면 기존 달력으로 폴백) ──
  if(typeof attV3ShouldRender==='function'){
    try{
      if(attV3ShouldRender()){ renderAttV3(); return; }
      // v3 비활성/미해당: v3 컨테이너 숨기고 레거시 영역 복원
      var _v3host = document.getElementById('att-v3');
      if(_v3host) _v3host.style.display = 'none';
      var _v3cal = document.getElementById('cal-area');
      if(_v3cal && !document.getElementById('attv3-month-overlay')) _v3cal.style.display = '';
      var _v3tp = document.getElementById('today-panel');
      if(_v3tp) _v3tp.style.display = '';
    }catch(e){ /* v3 오류 시 레거시 달력으로 계속 */ }
  }
  // ── 직종 분기 ──
  ...
  for(let d=1;d<=dim;d++){
    ...
    if(data&&data.status&&data.status!=='none'){
      ...
      if(wt==='day' && (s==='work'||s==='early') && data.start!==undefined && data.start > dayStart){
        ...
        html+=`<div style="position:absolute;top:3px;left:3px;font-size:9px;background:var(--mn-att-late, #e06010);color:#fff;padding:1px 4px;border-radius:3px;font-weight:700;">지각${lateRnd*60|0}분</div>`;
      }
      // ── 조퇴 공제 배지 ──
  ...
  document.getElementById('stats-row').innerHTML = `
    <div style="width:100%;display:flex;gap:10px;flex-wrap:wrap;">
      ...
      <div style="flex:2;min-width:200px;align-self:flex-start;background:var(--surface);border:1px solid var(--border);border-radius:14px;padding:16px 20px;">
        ...
        ${wDays === 0 ? `
        <!-- 신규 사용자: 기록 없음 -->
        <div style="font-size:11px;color:var(--text3);font-weight:600;letter-spacing:.5px;margin-bottom:10px;">예상 실수령액</div>
        <div style="background:rgba(79,124,255,.07);border:1px dashed rgba(79,124,255,.3);border-radius:10px;padding:12px 14px;text-align:center;">
          <div style="font-size:13px;font-weight:700;color:var(--text2);margin-bottom:4px;">📋 아직 기록이 없어요</div>
          <div style="font-size:12px;color:var(--text3);line-height:1.7;">오늘 날짜를 눌러 <b style="color:var(--accent);">출근 기록</b>을 남기면<br>이번 달 예상 급여가 자동으로 계산돼요</div>
        </div>
        ` : `
        <!-- 기록 있음: 실제 데이터 -->
        <div style="display:flex;justify-content:space-between;align-items:flex-start;gap:8px;margin-bottom:6px;">
          <div style="font-size:11px;color:var(--text3);font-weight:600;letter-spacing:.5px;line-height:1.5;">예상 실수령액<br>
            <span style="font-weight:400;opacity:.85;">이번달 ${wDays}일 기록 기준</span>
          </div>
        ...
        <div style="font-size:28px;font-weight:900;font-family:'JetBrains Mono';color:var(--green);line-height:1.1;">
          ${netPay.toLocaleString()}<span style="font-size:14px;font-weight:600;margin-left:2px;">원</span>
        </div>
        ${prevPay > 0 ? `<div style="font-size:11px;margin-top:5px;color:${diffColor};">${diffSign}${diff.toLocaleString()}원 <span style="color:var(--text3);">전월 대비</span></div>` : ''}
        ...
        <div style="margin-top:10px;">
          <div style="display:flex;justify-content:space-between;font-size:10px;color:var(--text3);margin-bottom:2px;">
            <span style="font-weight:600;">이번달 날짜 진행률</span><span>${progress}%</span>
          </div>
          ...
          <div style="font-size:9px;color:var(--text3);margin-top:2px;">오늘 날짜 기준 · 근무 기록과 무관</div>
        </div>
      </div>
...
function updatePrev(){
  ...
  if(wt==='day' && (pSt==='work'||pSt==='early') && s > dayStart){
    ...
    const lateRnd = Math.ceil(lateRaw / 0.5) * 0.5;
    lateTxt = `<div style="font-size:11px;color:var(--mn-att-late, var(--orange));margin-bottom:3px;">⚠️ 지각 ${Math.round(lateRaw*60)}분 → 30분 단위 올림 <b>${lateRnd*60|0}분 공제</b> (-${fmt(lateRnd*hourlyRate)})</div>`;
  }
...
function budgetSave(){
  ...
  if(typeof HomeStage!=='undefined') HomeStage.advance(3);
}
...
// ════════════════════════════════════════════════════════
// v3.2: 생존관리 시급제수입 행 — 사업장 2개 이상이면 사업장별로 분해 표시
function _bdgEmployeeRows(income){
  const row = (label, val, sub) => `<div style="display:flex;justify-content:space-between;font-size:13px;color:var(--text2);margin-bottom:4px;"><span>${label}</span><b style="color:var(--text);">${val}</b></div>`;
  try{
    if(typeof CompanyEngine!=='undefined' && CompanyEngine.isMulti()){
      const all = CompanyEngine.getPayDataAll();
      let h = '';
      all.breakdown.forEach(b => {
        h += row((b.wpId===activeWpId?'🏢 ':'📦 ')+b.name, b.finalPay.toLocaleString()+'원');
      });
      return h;
    }
  }catch(e){}
  return row('🏢 시급제수입', (typeof _hasAttendance==='function'&&!_hasAttendance())?'<span style="color:var(--text3);font-weight:400;">기록 없음</span>':income.employee.toLocaleString()+'원');
}

function getIncomeSummary(y, m){
  let employee=0, alba=0, freelancer=0, etc=0, salary=0;
  ...
  const isSalary = selectedJobs.indexOf('salary')>=0;
  ...
  try{
    if(isEmployee && typeof getPayData==='function'){
      ...
      // v3.0: 사업장 2개 이상이면 CompanyEngine이 기존 계산 엔진을 사업장별 반복 호출해 합산
      if(isCurMonth && typeof CompanyEngine!=='undefined' && CompanyEngine.isMulti()){
        employee = CompanyEngine.getPayDataAll().total || 0;
      } else if(!isCurMonth && typeof CompanyEngine!=='undefined' && CompanyEngine.isMulti() && typeof getPayDataForMonth==='function'){
        let sum = 0;
        CompanyEngine.companies().forEach(c => {
          try{ const p = CompanyEngine.runFor(c.wpId, c.empId, () => getPayDataForMonth(y,m)); if(p) sum += p.finalPay||0; }catch(e){}
        });
        employee = sum;
      } else {
        // v3.3.1: 기록 없는 달의 "설정 기반 예상치" 합산 방지 — 기록이 있을 때만 수입으로 집계
        const _hasRec = (typeof CompanyEngine!=='undefined' && typeof activeWpId!=='undefined' && activeWpId)
          ? CompanyEngine.hasRecords(activeWpId, activeEmpId, y, m)
  ...
  // v2.0: 연봉제 — 근태와 무관한 고정 월 실수령액 (연봉 미설정 시 0)
  try{
    if(isSalary && typeof getSalaryPayData==='function'){
      const sd = getSalaryPayData();
      if(sd.configured) salary = sd.netPay || 0;
    }
  }catch(e){}
  ...
  return { employee, salary, alba, freelancer, etc, total: employee+salary+alba+freelancer+etc, selectedJobs };
}
...
// ══════════════════════════════════════════
// v4.1 생존관리 리디자인 — "이번 달을 버틸 수 있는지 5초 안에" (UI 전용, 계산 무변경)
// 데이터는 전부 기존 calcZeroBalanceDate()/getIncomeSummary() 값 재사용
// ══════════════════════════════════════════
function _bdgSurvivalHero(zb, incTotal, hasIncomeData, totalExpense, remain){
  try{
    const nick = (typeof memName!=='undefined' && memName) ? memName+'님, ' : '';
    const nodata = zb.riskLevel==='nodata' || !hasIncomeData;
    // 생존 점수 = 100 - 가용예산 사용률 (표시 전용 합성 지표)
    const score = nodata ? null : Math.max(0, Math.min(100, 100 - zb.spentPct));
    const riskMap = {
      safe:        { mood:'celebrate', color:'var(--mn-success)', bar:'mn-bar--good', label:'안전',   msg: nick+'현재 상태라면 이번 달을 <b>안전하게 버틸 수 있어요</b>.' },
      warning:     { mood:'thinking',  color:'var(--mn-warning)', bar:'mn-bar--warn', label:'주의',   msg: nick+'<b>주의가 필요해요.</b> 예상보다 지출이 조금 많아요.' },
      danger:      { mood:'warn',      color:'var(--mn-error)',   bar:'mn-bar--bad',  label:'위험',   msg: nick+'<b>지출이 많이 빨라요.</b> 지금부터 아껴야 해요.' },
      danger_high: { mood:'error',     color:'var(--mn-error)',   bar:'mn-bar--bad',  label:'초위험', msg: nick+'<b>가용예산을 이미 넘었어요.</b> 함께 대책을 세워봐요.' },
      nodata:      { mood:'unknown',   color:'var(--text3)',      bar:'',             label:'-',      msg: nick+'수입과 지출을 기록하면 <b>생존 분석</b>을 시작할게요.' }
    };
    const rk = riskMap[nodata ? 'nodata' : zb.riskLevel] || riskMap.nodata;

    let H = '';
    // 브랜드 문구 — 머니냥의 정체성
    H += `<div class="mn-caption" style="text-align:center;padding:2px 0 8px;">머니냥은 이번 달을 <b style="color:var(--mn-brand);">끝까지 버틸 수 있도록</b> 도와드립니다</div>`;

    // ① 생존 Hero — 캐릭터 + 생존 점수
    H += `<div class="mn-card mn-fade-in" style="text-align:center;">
      ${(typeof MnCharacter!=='undefined')?MnCharacter.img(rk.mood,'lg',{animate:'pop'}):''}
      <div style="font-size:var(--font-base);color:var(--text);line-height:1.6;margin-top:8px;">${rk.msg}</div>
      ${score!==null ? `<div style="margin-top:10px;"><span style="font-size:calc(34px * var(--ui-scale));font-weight:900;font-family:'JetBrains Mono';color:${rk.color};">${score}</span><span style="font-size:var(--font-md);font-weight:800;color:${rk.color};">점</span>
        <span class="mn-badge" style="background:${rk.color};color:#fff;margin-left:8px;vertical-align:6px;">${rk.label}</span></div>` : ''}
    </div>`;
    ...
    if(score!==null){
      H += `<div class="mn-card">
        <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:8px;">
          <span class="mn-h" style="margin:0;">🛡️ 생존지수</span>
          <b style="color:${rk.color};font-size:var(--font-md);">${score}% · ${rk.label}</b></div>
        <div class="mn-bar ${rk.bar}" style="height:14px;border-radius:7px;"><i style="width:${score}%;"></i></div>
      </div>`;
    }

    // ③ 이번 달 현황 (수입/지출/남은 돈)
    H += `<div class="mn-card"><div style="display:grid;grid-template-columns:1fr 1fr 1fr;text-align:center;gap:6px;">
      <div><div class="mn-caption">예상수입</div><b style="font-size:var(--font-sm);color:var(--text);">${hasIncomeData?fmt(incTotal):'—'}</b></div>
      <div style="border-left:1px solid var(--border);border-right:1px solid var(--border);"><div class="mn-caption">예상지출</div><b style="font-size:var(--font-sm);color:var(--mn-error);">${fmt(totalExpense)}</b></div>
    ...
    if(!nodata && zb.daysLeft !== null && zb.daysLeft !== undefined){
      ...
      H += `<div class="mn-card ${ok?'mn-card--good':'mn-card--warn'}" style="text-align:center;">
        <div class="mn-caption">📆 버틸 수 있는 날짜</div>
        <div style="font-size:var(--font-lg);font-weight:800;color:${ok?'var(--mn-success)':'var(--mn-error)'};margin-top:3px;">
          ${zb.date==='이미 부족해요' ? '이미 부족해요' : '현재 소비라면 '+zb.date+'까지 안전해요'}</div>
        ${zb.avgDailySpend>0?`<div class="mn-caption" style="margin-top:3px;">하루 평균 지출 ${fmt(zb.avgDailySpend)}</div>`:''}
      </div>`;
    }

    // ⑤ AI 생존 조언 — 하루 10% 절약 시 늘어나는 생존일 (기존 수치만 재조합)
    if(!nodata && zb.avgDailySpend>0 && zb.daysLeft!==null && zb.daysLeft>=0){
      const saved = Math.round(zb.avgDailySpend*0.1);
      const newDays = Math.floor(zb.currentBalance / (zb.avgDailySpend*0.9));
      const gain = newDays - zb.daysLeft;
      if(gain > 0){
        H += `<div class="mn-card mn-card--flat" style="display:flex;gap:10px;align-items:flex-start;background:var(--mn-brand-soft);border:none;">
          ...
          <div style="font-size:var(--font-sm);color:var(--text);line-height:1.6;">하루 지출을 <b>10%(약 ${fmt(saved)})</b>만 줄이면<br>버틸 수 있는 날이 <b style="color:var(--mn-success);">+${gain}일</b> 늘어나요.</div>
        </div>`;
      }
    }

    // ⑥ 위험 알림 + 추천 행동
    if(zb.riskLevel==='danger' || zb.riskLevel==='danger_high'){
      const oneDay = (typeof companyRate!=='undefined' && companyRate>0) ? Math.round(companyRate*8) : 0;
      H += `<div class="mn-card" style="border-left:3px solid var(--mn-error);">
        <div style="font-size:var(--font-base);font-weight:800;color:var(--mn-error);margin-bottom:6px;">🔴 이대로면 월급날 전에 부족할 수 있어요</div>
        <div style="font-size:var(--font-sm);color:var(--text2);line-height:1.7;">추천<br>• 변동지출 줄이기 (아래 카테고리에서 큰 항목부터)<br>${oneDay>0?`• 추가 근무 1일 → 약 <b style="color:var(--mn-success);">${fmt(oneDay)}</b> 확보`:''}</div>
      </div>`;
    }
    return H;
  }catch(e){ return ''; }
...
function renderBudgetPage(){
  ...
  const income = getIncomeSummary(y, m);
  // ★ Home(weather.js financial())과 동일 정책: 출근 기록 없는 직장인의 finalPay는
  //   기본급 가정치라 실제 수입이 아님 — 표시 판단용 incTotal에서만 제외(계산 로직 비변경)
  let _bdgIncTotal = income.total;
  try{
    if(typeof _hasAttendance==='function' && !_hasAttendance() && income.selectedJobs.indexOf('employee')>=0){
      const _pd = (typeof getPayData==='function') ? getPayData() : null;
      if(_pd) _bdgIncTotal = Math.max(0, _bdgIncTotal - (_pd.finalPay||0));
    }
  }catch(e){}
  const hasIncomeData = _bdgIncTotal > 0;
  const fixedTotal = Object.values(budgetState.fixedExpenses).reduce((s,v)=>s+(parseInt(v)||0),0);
  const ymPrefix = `${y}-${String(m+1).padStart(2,'0')}`;
  const monthVar = (budgetState.variableExpenses||[]).filter(e=>e.date && e.date.startsWith(ymPrefix));
  const varTotal = monthVar.reduce((s,e)=>s+(parseInt(e.amount)||0),0);
    ...
    <div class="budget-container">
      ...
      <div class="budget-grid">
        ...
        <div class="budget-card">
          ...
          ${(income.selectedJobs.includes('employee')||income.selectedJobs.length===0)?_bdgEmployeeRows(income):''}
          ${income.selectedJobs.includes('salary')?`<div style="display:flex;justify-content:space-between;font-size:13px;color:var(--text2);margin-bottom:4px;"><span>💼 연봉제수입</span><b style="color:var(--text);">${(income.salary||0)>0?(income.salary||0).toLocaleString()+'원':'<span style="color:var(--text3);font-weight:400;">연봉 미설정</span>'}</b></div>`:''}
          ${(income.selectedJobs.some(j=>['convenience','shortAlba'].includes(j))||income.selectedJobs.length===0||income.alba>0)?`<div style="display:flex;justify-content:space-between;font-size:13px;color:var(--text2);margin-bottom:4px;"><span>💪 알바수입${income.alba>0?' <span style="font-size:10px;color:var(--text3);">(주휴수당 포함 가능)</span>':''}</span><b style="color:var(--text);">${income.alba.toLocaleString()}원</b></div>`:''}
          ${(income.selectedJobs.includes('freelancer')||income.selectedJobs.length===0||income.freelancer>0)?`<div style="display:flex;justify-content:space-between;font-size:13px;color:var(--text2);margin-bottom:4px;"><span>💻 프리랜서수입</span><b style="color:var(--text);">${income.freelancer.toLocaleString()}원</b></div>`:''}
          ${income.etc>0||income.selectedJobs.length===0?`<div style="display:flex;justify-content:space-between;font-size:13px;color:var(--text2);margin-bottom:4px;"><span>${income.selectedJobs.some(j=>['delivery','driver'].includes(j))&&!income.selectedJobs.includes('employee')?'🛵 배달·대리수입':'➕ 기타수입'}</span><b style="color:var(--text);">${income.etc.toLocaleString()}원</b></div>`:''}
          <div style="display:flex;justify-content:space-between;font-size:14px;font-weight:700;border-top:1px solid var(--border);padding-top:8px;margin-bottom:6px;"><span>총수입</span><span style="color:${hasIncomeData?'var(--green)':'var(--text3)'};">${hasIncomeData?income.total.toLocaleString()+'원':'<span style="font-weight:400;font-size:13px;">수입 입력 후 계산</span>'}</span></div>
          ...
          <div style="display:flex;justify-content:space-between;font-size:17px;font-weight:800;border-top:1px solid var(--border);padding-top:8px;"><span>남은금액</span><span style="color:${hasIncomeData?(remain>=0?'var(--accent)':'var(--red)'):'var(--text3)'};">${hasIncomeData?remain.toLocaleString()+'원':'<span style="font-weight:400;font-size:13px;">수입 입력 후 계산</span>'}</span></div>
        </div>
